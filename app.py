...
st.markdown(
    """
    Данное демо-приложение рассчитывает **приблизительный** риск развития ВРГН
    на основании четырёх входных параметров комет-анализа и одного генетического фактора (GSTP1 rs1695).
    ---
    **Логика расчёта**:
    - Если регион **без экотоксикантов**: риск = 0.0015 (0.15%).
    - Если регион **с экотоксикантами**:
      1. Для каждого из параметров (длина хвоста, % ДНК, момент хвоста) риск пропорционально
         интерполируется между 0.0015 (минимум) и 0.00365 (максимум).
      2. Итог = среднее трёх значений.
    - Если генотип (наличие G-аллеля по rs1695) – полученный риск умножается на 3.2.
    ---
    Значения риска показываются в процентах, а ниже шкала от минимального до максимального.
    """
)
